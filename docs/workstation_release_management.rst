...
Releasing an rc package is the first step before you begin QA or any signing ceremonies. Even when you are releasing a hotfix, it is recommended to start with an rc package to defer needing the Securedrop release key until the day of release when you are ready to deploy the package to production.
...
3. Push an rc tag on your new commit. We will be building from this tag in the next step.
...
1. Merged the ``release`` branch into ``main`` to deploy your package to https://apt.freedom.press.
2. Now that your package is on https://apt.freedom.press, you can run the updater to install your package in a production environment and ensure that it works as expected.
